...
2. 在“设置 → Codex 配置”顶部复制本地 Base URL 和 API Key。
...
最省事的方式是在“设置 → Codex 配置”点击“接管 Codex”。Aether 会把当前用户的 Codex Provider 指向本地代理，并保留接管前的本机配置用于恢复。详情见 [Codex 配置与会话统一](#codex-配置与会话统一)。
...
| 页面 | 现有能力 |
| --- | --- |
| 上游管理 | 代理状态与全局统计、OAuth/API Key 上游、搜索筛选、导入导出、账号编辑、用量、额度价值测算、启停、连接测试、优先级、并发容量、回收站 |
| Team 取号 | 从 sub2api SUPPLY API 下单领取 Team 账号（team_1h）、余额/库存/单价概览、订单状态跟踪，完成后自动下载 Sub2 JSON 并导入账号池 |
| 渠道监控 | 24 小时/7 天可用率、首包延迟、失败、估算费用、容量和最近观测，支持轻量连接检测与中转站模型验真 |
| 市场监控 | 商品比价、店铺状态、行情分析、提醒规则、未读事件和系统通知 |
| 请求日志 | 按状态、关键字和模型一致性筛选真实上游尝试，查看渠道、路由、请求模型、上游响应模型、Token、费用、首包和总耗时 |
| 设置 | 开机自启动、成本保护、Codex 出站身份与设备指纹收敛、出站代理、图片生成上游、Team 取号 Token、Codex 配置接管/会话迁移/提示词/Skills、版本信息、回收站和反馈邮件模板 |
...
| API Key 中转站 | 多站点接入、兼容路径转发、通用及 New API 用量查询、站点快速打开、成本倍率同步 | 转发能力取决于中转站；倍率同步仅兼容 Sub2API 账单协议 |
| 混合调度 | 模型白名单、优先级、平滑加权、会话粘性、容量、速率限制、分类冷却、流式断流隔离、可选成本保护 | 单个请求最多尝试 3 个上游，并受 75 秒启动预算和 25 秒首事件超时限制；每个账号默认 10 次/秒速率上限；不保证请求零中断 |
| 用量与费用 | OAuth 5 小时/7 天窗口、按当前周期实测费用推算账号额度、中转站消费/余额、Token 分项、费用估算 | 上游未返回或格式不兼容时无法展示；未知模型 Token 不计价；本地推算不是卖家承诺或官方账单 |
...
| Team 取号 | 通过 sub2api SUPPLY API 下单领取 Team 账号，完成后自动下载 Sub2 JSON 并导入 | 依赖 `bugteam.team` 服务与 Customer Token；订单按 FIFO 履约，本地最多保留 30 条记录 |
...
2. Aether 会打开内置授权 Tab；在 OpenAI 页面登录并确认授权。
...
如本机运行了代理软件，可在“设置 → 出站代理”开启 Aether 的出站代理；默认地址为 `http://127.0.0.1:7890`。它会立即用于 OAuth 授权网页、令牌兑换、刷新、额度查询、账号检测和本地中转上游请求，无需虚拟网卡。内置授权页及其新开的登录窗口会继承 HTTP、SOCKS5 或 SOCKS5H 代理；WebView2 不支持 HTTPS 代理传输，因此 HTTPS 代理只用于后端请求，授权页会明确报错而不会静默直连。

“设置 → Codex 出站身份”默认每 6 小时从官方 `openai/codex` Release 同步最新稳定版。OAuth 转发、Models、额度查询、账号检测、授权兑换和令牌刷新共用该版本构造 `codex-tui` User-Agent、`originator` 与 `version`，并保持 UA 首尾版本一致，避免陈旧或互相矛盾的客户端身份进入上游降载分桶；也可以关闭自动同步或手动立即同步。
...
- 名称。
- API Key。
- Base URL，例如 `https://api.example.com/v1`；QuantumNous/new-api 建议填写站点根地址，例如 `https://new-api.example.com`。
- 模型白名单，例如 `gpt-5,gpt-5-mini`；留空表示允许全部模型。
- 优先级 `0-1000`，数字越小越优先。
- 权重 `1-1000`，用于同一优先级内的新请求分配。

如果站点实现了当前兼容的 Bearer `GET /v1/usage`，Aether 可以展示消费、限额、订阅或钱包余额。QuantumNous/new-api 会自动识别 `GET /api/usage/token/`，读取 Token 原始额度，并通过 `/api/status` 的 `quota_per_unit` 显示近似美元值；同时读取 `/api/log/token` 的受限最近日志摘要。用量接口不兼容不会阻止正常 API 转发。
...
- 编辑已添加账号的名称、优先级、权重、并发和成本倍率；API Key 中转站还可更换 Key、Base URL 和模型白名单，OAuth 账号可一键同步 Codex 模型列表。
- OAuth 查询额度后，优先用当前 7d 周期（样本不足时用 5h）内经过本应用的成功请求费用除以已用比例，显示账号额度近似值；无有效样本时显示“额度待测”。
- OpenAI 返回重置额度明细时，账号用量卡会展示剩余次数与最近到期时间；缓存中的过期额度会在读取时自动剔除。
...
“Team 取号”是一个顶级工作区标签页，通过 sub2api 的 SUPPLY API 从 `bugteam.team` 下单领取 `team_1h`（Team 1 小时）账号，订单完成后自动下载 Sub2 JSON 并导入 Aether 账号池。

### 前置配置

- 在“设置 → Team 取号”填写 Customer Token。Token 只保存在本机数据库，由 Rust 后端直接以 `X-Customer-Token` 头发送给取号服务，不会写入请求日志。
- 未配置 Token 时，取号页会引导打开设置。

### 下单与履约

- 页面展示可用余额、当前库存、预计单价和本次数量锁款估算。
- 创建订单时按 FIFO 自动履约；订单会携带幂等键，提交超时后再次操作会复用同一幂等键，避免重复扣款。
- 数量范围为 1–1000；订单状态（提交中 / 已下单 / 等待补货 / 锁款中 / 已完成 / 失败等）每 3 秒轮询一次。
- 本地最多保留最近 30 条订单记录。

### 自动导入

订单完成后，Aether 自动请求 `format=sub2` 下载账号数据，走与“上游导入”相同的解析与入库流程，并展示新增/更新/失败数量。下载或导入失败时可手动重试导入。

### 边界

- 该功能依赖 `bugteam.team` 服务、库存与余额状态；库存/价格/余额仅作下单参考，不是可用性承诺。
- 下单会真实扣减余额，请先确认数量与锁款金额。
...
模型白名单不区分大小写并支持 `*` 通配符；留空表示允许全部模型。OAuth 账号按账号级容量限制；API Key 中转站除账号级容量外，还按站点（Base URL）共享一套并发容量。
...
Aether 会针对 `401-403`、`408`、`429`、带模型信息的 `404`、`5xx` 和传输错误应用账号级或模型能力级冷却，`429` 优先遵循有效的 `Retry-After`。Codex 流内 `server_is_overloaded` / `slow_down` 视为请求级瞬时降载：前导帧尚未产生客户端输出时先在同账号短暂重试，仍失败再切换渠道；已经输出后无法切号时改写为客户端可重试的 `server_error`，不会把账号加入故障冷却。Responses 工具 Schema 中显式为 `null` 的 `parameters.type` 会在发往上游前修正为 `object`。OAuth Token 在请求前发现即将到期时会刷新，同一账号的并发刷新请求会合并处理；收到未授权响应后也会尝试强制刷新一次。

每个请求最多尝试 3 个上游，响应启动阶段总预算为 75 秒，上游首事件超时为 25 秒。每个账号默认按 10 次/秒做本地速率限制；API Key 中转站还按站点（Base URL）共享一套并发容量。流式响应的首个有效载荷一旦提交给客户端，后续断流无法无损切换并重放；Aether 会记录错误并暂时隔离对应渠道（约 20 秒）。
...
Responses SSE 会流式透传；OAuth 上游返回流式事件而客户端请求非流式时，Aether 可以聚合最终响应。统计会尽量记录输入、输出、缓存读取、缓存写入和推理 Token，并按响应模型的本地价格快照估算费用；未知模型会明确标记为未计价。请求日志还会记录上游响应声明的模型，并标记它是否与实际发往上游的模型一致。

Codex 客户端以 WebSocket 连接 Aether 时，Aether 会按上游类型选择协议：OpenAI/Codex OAuth 与官方 `api.openai.com` API Key 保持原生 Responses WebSocket；自定义 API Key 中转站将 `response.create` 转换为标准 HTTP `POST /v1/responses` 流式请求，再把 SSE `data` 事件逐帧送回客户端。HTTP/SSE 请求仍使用“设置 → 出站代理”的 HTTP、HTTPS、SOCKS5 或 SOCKS5H 网络路径。请求日志将该链路标记为 `WebSocket → HTTP SSE`。

## Codex 配置与会话统一

Codex 配置面板现已并入“设置”页顶部，与通用设置、出站代理、图片生成上游、Team 取号 Token 等同页管理。
...
设置页的“Codex 扩展”可以管理当前 Codex Home 下的资源：

- 提示词预设保存在 Aether 本地数据库；“保存并启用”会写入 `AGENTS.md`，也可把当前文件导入为预设。外部修改当前启用的 `AGENTS.md` 后，重新打开设置会回填到对应预设。
- Skills 扫描 `skills/*/SKILL.md`。停用时会把目录移动到 `.aether-disabled-skills`，重新启用时移回 `skills`，不删除 Skill 文件。
- 设置了 `CODEX_HOME` 时使用该目录；否则使用当前用户的 `~/.codex`。
...
- 新建 `market.db` 时会加入 27 家默认店铺；用户可以添加、编辑、暂停、启用或删除店铺。
- 店铺数据只在用户点击“立即刷新”时采集，不在后台自动轮询；连续手动刷新有 30 秒冷却。
- 店铺请求采用有限并发、请求间隔、分页、资料/费率缓存以及主备 API。
- 连续失败、`403`、`429` 或安全校验会触发店铺退避或全局熔断；保护期间继续显示已有缓存，保护结束后可再次手动刷新。
- 商品连续两次未出现后才确认售罄，降低短暂接口缺项造成的误报。

市场数据只在手动刷新时更新，不是交易所级实时行情。
...
WebView 会记录复制/剪切和 Clipboard API 写入事件，并在复制后检查本机剪贴板是否包含完整、受支持的 CPA/Sub2API JSON。WebView 下载会显式保存到系统下载目录，处理重名和非法文件名；下载完成后扫描不超过 10 MB 的 UTF-8 文件。识别成功只生成脱敏候选，仍需用户确认；Aether 不会把剪贴板或下载文件主动上传到自营服务。

OpenAI OAuth 来源的 WebView 会使用“设置 → 出站代理”中已启用的 HTTP、SOCKS5 或 SOCKS5H 代理。授权过程中打开的新窗口会继承同一 OAuth 来源和代理设置，回调完成后相关授权 Tab 会统一关闭。
...
- 上游响应声明的模型，以及请求模型与响应模型不一致的标记。
- 状态、HTTP 状态码、首包时间和总耗时。
- 输入、输出、缓存和推理 Token，以及估算费用。
- 已限制长度并脱敏的错误摘要。

WebView Tab 的下载会写入系统下载目录并在界面显示下载状态；下载完成后会自动检查是否为可导入的账号备份文件。

日志不保存请求/响应正文、完整凭据、查询参数或会话 ID。默认保留 30 天且最多 20,000 条；日志页支持关键字、状态筛选、仅看模型不一致、分页、立即刷新和手动清空。

### 渠道监控

渠道监控复用真实请求日志，并提供用户主动触发的轻量 Models 连接检测。API Key 中转站还可以手动执行模型验真：直连目标站点检查 `/models` 声明，并依次运行结构化输出、工具调用和多轮指令三组动态挑战，记录响应模型、有效探针、Token、耗时和逐项证据。应用不会为了维持健康曲线或验模历史在后台定时发送额外的付费模型请求。

- 成功、首包小于 6 秒且总耗时小于 20 秒：正常。
- 成功但首包达到 6 秒或总耗时达到 20 秒：响应较慢，计入异常渠道。
- 重试或错误：异常。
- 客户端取消和仍在进行的请求：不进入可用率与时间线。

页面提供 24 小时/7 天可用率、平均首包、尝试、失败、估算费用、当前容量和最近观测。全局摘要只统计已启用渠道；已停用渠道仍保留在列表中供查看。

验模结果分为“暂未发现异常、可疑、高风险、无法判断”，并为每个中转站保留最近 30 次历史。响应中的 `model` 字段可以被中转站伪造，能力相近的模型也可能通过相同挑战，因此结果只表示当前黑盒证据强弱；遇到异常应复测并与可信官方源对照。
...
- `sub2api.db`：账号、凭据、设置、请求日志、模型验真历史、用量缓存、Codex 接管快照等。
...
- 中转站用量：用户配置站点的 `/v1/usage`；New API 站点的 `/api/usage/token/`、`/api/status` 和 `/api/log/token`；开启倍率同步的 Sub2API 中转站还会请求 `/v1/sub2api/billing`。
- 市场监控：链动小铺 `www.ldxp.cn` / `pay.ldxp.cn`。
- 图片生成：专用图片生成上游（默认 `api.openai.com`）。
- Team 取号：sub2api SUPPLY API `bugteam.team`。
...
Codex 接管会读写 `~/.codex/auth.json` 和 `~/.codex/config.toml`；提示词管理会读写 `AGENTS.md`，Skills 启停会在 `skills` 与 `.aether-disabled-skills` 间移动目录；会话统一会读写 `sessions`、`archived_sessions` 和 `state_5.sqlite`。Aether 会创建恢复快照或文件备份，但重要会话仍建议额外备份。
...
- 本地费用、额度和余额展示仅用于辅助观察，不是官方账单或结算凭证。OAuth 账号额度价值测算只覆盖经过本应用且已成功计价的请求，早期样本较少时可能波动明显。
