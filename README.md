...
Aether 聚焦“个人本机上游管理”，不是公网中转站、计费平台或多租户 SaaS。当前版本为 `0.1.0-alpha.1`，仍在持续完善中。
...
- 删除账号进入回收站，可随时恢复或永久删除。
- 中转站可一键使用内嵌 WebView 打开站点首页，自动清除前缀为 `api.` 的子域名、API 路径、查询参数和认证信息。
...
- 内嵌 WebView 中下载的文件也会自动扫描，检测可导入的账号数据。
...
- 应用内置 WebView 标签页系统，可直接在应用内打开中转站或其他网页，无需切换到外部浏览器。
- 中转站"打开站点"使用内嵌 WebView，自动清除前缀为 `api.` 的子域名、API 路径、查询参数和认证信息。
- WebView 内注入复制审计脚本，拦截 copy/cut/Clipboard API，可自动识别并导入 CPA/Sub2API JSON。
- WebView 中的文件下载完成后自动扫描，检测可导入的账号数据。
- 支持 `Ctrl+Tab` / `Ctrl+PageDown/PageUp` 切换标签、`Ctrl+T` 打开标签选择器、`Ctrl+W` 关闭标签、`Ctrl+1-9` 跳转到指定标签。

### 回收站与软删除

- 账号删除采用软删除机制，删除后进入回收站而非立即销毁。
- 回收站支持查看、恢复和永久删除，也可一键清空全部回收站内容。
- 软删除账号不影响调度，恢复后立即重新参与路由。

### 市场监控

- 内置市场监控模块，从上游数据源定时拉取商品和店铺信息，提供商品分类、价格分析和库存趋势。
- 支持告警系统：商品不可用、店铺健康异常、库存激增和价格异常等多种告警类型，含严重级别和免打扰时段设置。
- 告警触发时通过系统原生通知推送，点击通知可聚焦应用窗口并跳转到对应页面。
- 店铺管理支持添加、启停和删除，内置熔断保护机制防止对数据源的过度请求。
- 提供分析面板，展示商品时序数据、价格趋势和店铺健康状态。

### 费用与预算管理

- 支持设置每日预算上限（USD），超出后在代理信息面板中提示。
- OAuth 额度支持配置自动刷新间隔（5/15/30/60 分钟），开关和间隔持久化到本地数据库。
- 全局 Token 和费用累计统计独立于单个账号，可在代理信息面板查看。

### 网络与链路优化

- 内置自定义可信 DNS 解析器，使用 Cloudflare（1.1.1.1/1.0.0.1）和 Google（8.8.8.8/8.8.4.4）公共 DNS 绕过本地 DNS 污染。
- 所有上游请求均通过自定义 DNS 解析器构建的 HTTP 客户端发出，不依赖系统 DNS 缓存。
- Codex 代理响应自动解析 `x-codex-primary/secondary-*` 配额头部，深度合并到额度缓存中，提供更精确的用量感知。
...
| 托盘、状态开关、站点快速打开和 Codex 接管 | 已完成 | 站点使用内嵌 WebView 打开 |
| Codex 会话统一 | 已完成 | 新会话进入 `custom` 桶，既有历史可显式迁移和恢复 |
| 内嵌 WebView 工作区 | 已完成 | 复制审计、下载监控、标签页管理 |
| 回收站与软删除 | 已完成 | 删除进入回收站，支持恢复和永久删除 |
| 市场监控与告警 | 已完成 | 定时拉取、分类、告警、原生通知、熔断保护 |
| 费用预算与自动刷新 | 已完成 | 每日预算、自动刷新间隔、全局统计 |
| 自定义 DNS 解析 | 已完成 | Cloudflare + Google 公共 DNS，绕过本地污染 |
...
- **内嵌浏览器工作区**：中转站和其他网页可直接在应用内打开，复制审计和下载监控自动识别可导入数据。
- **市场感知能力**：内置市场监控和告警通知，不必依赖外部工具即可跟踪上游商品动态。
- **抗 DNS 污染**：内置可信 DNS 解析器，无需额外配置即可绕过本地 DNS 污染。
...
- 完善 Stronghold 加密保险库的前端集成，提供加密备份和凭据保护。
- 完善市场监控的数据源兼容性和告警规则可配置性。
...
| 阶段 | 计划 |
| --- | --- |
| 近期 | 完成 Stronghold 加密保险库的前端集成，将账号凭据从 SQLite 明文迁移到加密存储，提供加密备份 |
| 近期 | 增加账号和中转站完整编辑，将已分环境持久化的代理端口、超时与路由参数开放到设置界面 |
| 近期 | 完善市场监控的数据源兼容性和告警规则可配置性 |
...
- 账号凭据目前以明文存放在当前用户的 Tauri 应用数据目录 SQLite 中；后端已实现 Stronghold 加密保险库模块，前端集成正在进行中。
...
- 内嵌 WebView 与外部网页共享相同的网络环境，WebView 中的复制审计和下载监控仅处理本地剪贴板和文件，不会主动上传数据。
- 市场监控数据仅保存在本机独立 SQLite 数据库中，告警事件和时序分析不上传到外部服务；数据源请求受熔断保护限制。
