...
| OpenAI/Codex OAuth | 多账号导入、刷新、额度、Responses/Models 转发 | 当前只支持 OpenAI/Codex，不支持 Claude/Gemini Provider，也没有应用内 OAuth 授权流程 |
| API Key 中转站 | 多站点接入、兼容路径转发、用量查询、站点快速打开 | 转发能力取决于中转站；用量只兼容 Bearer `GET /v1/usage` 的已知响应字段 |
| 混合调度 | 模型白名单、优先级、平滑加权、会话粘性、容量、速率限制、分类冷却 | 单个请求最多尝试 3 个上游，并受 30 秒启动预算限制；不保证请求零中断 |
...
> 导出的备份包含完整 OAuth Token、API Key、模型白名单、优先级、权重和并发配置，应按密码文件保管。
...
3. 优先使用数字更小的优先级。
4. 同一优先级内按权重执行平滑加权轮询。
5. 当前上游在响应提交前失败时，在剩余启动预算内尝试备用上游。
...
- 当前调度不会根据 OAuth 剩余额度、中转钱包余额、每日预算或估算成本自动选站。
