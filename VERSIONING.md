...
| 阶段 | 含义 | 进入条件 |
|---|---|---|
| `alpha.N` | 早期测试，功能、数据格式和交互可能有破坏性变更 | 核心流程可运行，仍在快速迭代 |
...
同时更新 `src/lib/commands.ts` 中 `get_app_version` 的前端预览数据，确保 `pnpm dev` 展示的版本与当前发布版本一致；该值不参与正式包构建元数据。
