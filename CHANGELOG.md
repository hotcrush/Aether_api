...
- 优化界面渲染性能：降低代理统计轮询频率，并减少请求日志与网页标签页的重复渲染
- 移除长列表与卡片上的磨砂背景并优化列表行渲染，降低大数据量下的渲染开销
- 优化 Team 取号页：新增官网入口，改进头部布局与移动端响应式

## [0.1.0] - 2026-08-15

### Changed

- 全面采用现代 iOS 视觉语言，统一导航、页面标题、信息卡片、表格、设置分组、弹窗和状态反馈
- 为按钮、标签、分段控件、列表项和卡片增加一致的按压、弹簧和切换动效
- 为市场监控、渠道监控、请求日志、Codex 扩展和 Team 取号页面应用统一的磨砂分组界面
- 优化输入框、滚动条、进度条、徽章和聚焦状态，并自动适配系统减少动态效果设置
